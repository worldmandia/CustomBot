...
public class SEND_EMBED implements Action {
    ...
    public void run(GenericInteractionCreateEvent event) {
        event.getMessageChannel().sendMessageEmbeds(new EmbedBuilder().setTitle(title).setDescription(description).build()).queue();
    }

    @Override
    public void runWithPlaceholders(GenericInteractionCreateEvent event, StringSubstitutor str) {
        event.getMessageChannel().sendMessageEmbeds(new EmbedBuilder().setTitle(str.replace(title)).setDescription(str.replace(description)).build()).queue();
    }
