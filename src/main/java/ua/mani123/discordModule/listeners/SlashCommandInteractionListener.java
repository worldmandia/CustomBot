...
public class SlashCommandInteractionListener extends ListenerAdapter implements DiscordListener {
    ...
    public void onSlashCommandInteraction(@NotNull SlashCommandInteractionEvent event) {
        discordUtils.getDiscordConfigs().getInteractions().stream().filter(interaction -> interaction instanceof COMMAND_INTERACTION).forEach(interaction -> interaction.run(event, new TempData()));
    }
